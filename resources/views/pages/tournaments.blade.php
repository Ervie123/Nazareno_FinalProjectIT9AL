...
<div class="results-count" id="tCount">
  Showing {{ $tournaments->count() }} of {{ $total }} results
</div>

<!-- CARDS GRID — horizontal 3-column layout -->
<div class="card-grid" id="tGrid">
  @forelse($tournaments as $tournament)
    <div class="t-card" data-name="{{ strtolower($tournament->name) }}"
      ...
      <div class="t-card-foot">
        <span>Teams: <strong>{{ $tournament->teams_count }}</strong></span>
        <span>Matches: <strong>{{ $tournament->matches_count }}</strong></span>
      </div>

      <div class="t-card-actions">
        <button class="btn-edit-outline" onclick="openEditTournament({{ $tournament->id }})">
...
<div class="modal-backdrop hidden" id="tournamentModal">
  <div class="modal-box">
    ...
    <div id="tournamentModalBody">
      ...
      <form method="POST" id="tournamentForm" action="{{ route('tournaments.store') }}">
        @csrf
