...
<form method="GET" action="{{ route('matches.index') }}" class="toolbar">
  ...
  <select class="filter-sel" name="tournament" onchange="this.form.submit()">
    ...
    @foreach($tournaments as $t)
      ...
      @endphp
      <option value="{{ $value }}"
        {{ request('tournament') === $value ? 'selected' : '' }}>
        {{ $value }}
      </option>
    @endforeach
  </select>
  ...
  <select class="filter-sel" name="sort" onchange="this.form.submit()">
    <option value="date" {{ request('sort','date') === 'date' ? 'selected' : '' }}>Sort: Date</option>
    <option value="status" {{ request('sort') === 'status' ? 'selected' : '' }}>Sort: Status</option>
  </select>

  @if(request()->hasAny(['search','status','tournament','sort']))
    <a href="{{ route('matches.index') }}" class="btn-clear">✕ Clear</a>
  @endif
</form>
    ...
    <div class="match-card">
      ...
      <div class="match-card-top">
        ...
        <div class="match-card-btns">
          ...
          <form method="POST" action="{{ route('matches.destroy',$m['id']) }}"
      ...
      <div class="match-venue-row">
        {{ $m['venue'] }} · 
        {{ is_object($m['tournament']) ? $m['tournament']->name : ($m['tournament'] ?? '-') }}
      </div>
...
<div class="modal-backdrop hidden" id="createMatchModal">
  <div class="modal-box">
    ...
    <form method="POST" action="{{ route('matches.store') }}">
      @csrf
      @include('partials.match-form', ['tournaments'=>$tournaments])
...
<div class="modal-backdrop hidden" id="editMatchModal{{ $m['id'] }}">
  <div class="modal-box">
    <button class="modal-x"
      onclick="document.getElementById('editMatchModal{{ $m['id'] }}').classList.add('hidden')">✕</button>

    <div class="modal-title">Edit Match</div>

    <form method="POST" action="{{ route('matches.update',$m['id']) }}">
      @csrf @method('PUT')
      @include('partials.match-form', ['match'=>$m,'tournaments'=>$tournaments])
