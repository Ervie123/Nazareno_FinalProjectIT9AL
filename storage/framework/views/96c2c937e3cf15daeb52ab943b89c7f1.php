...
<div class="toolbar">
  ...
  <select class="filter-sel" id="matchTourn" onchange="filterMatches()">
    ...
    <?php $__currentLoopData = $tournaments; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $t): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
      <option value="<?php echo e(strtolower($t->name)); ?>"><?php echo e($t->name); ?></option>
...
<div class="results-count" id="matchCount">
  Showing <?php echo e($matches->count()); ?> of <?php echo e($matches->count()); ?> results
</div>

<!-- STATUS TABS — horizontal tabs -->
<div class="match-tabs">
  <button class="match-tab active" onclick="setMatchTab(this, 'all')">All (<?php echo e($matches->count()); ?>)</button>
  <button class="match-tab" onclick="setMatchTab(this, 'live')">Live (<?php echo e($matches->where('status','live')->count()); ?>)</button>
  <button class="match-tab" onclick="setMatchTab(this, 'scheduled')">Scheduled (<?php echo e($matches->where('status','scheduled')->count()); ?>)</button>
  <button class="match-tab" onclick="setMatchTab(this, 'completed')">Completed (<?php echo e($matches->where('status','completed')->count()); ?>)</button>
</div>
...
<div class="match-grid" id="matchGrid">
  <?php $__empty_1 = true; $__currentLoopData = $matches; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $match): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); $__empty_1 = false; ?>
    <div class="match-card" data-status="<?php echo e($match->status); ?>"
         data-teams="<?php echo e(strtolower($match->team_a . ' ' . $match->team_b)); ?>"
         data-venue="<?php echo e(strtolower($match->venue)); ?>"
         data-tournament="<?php echo e(strtolower($match->tournament)); ?>">
      <div class="match-card-top">
        <span class="badge badge-<?php echo e($match->status); ?>"><?php echo e($match->status === 'live' ? 'LIVE' : strtoupper($match->status)); ?></span>
        <div class="match-meta">
          ...
          <span>
            ...
            <?php echo e($match->match_time); ?>

          </span>
        </div>
        <div class="match-card-btns">
          <button class="btn-tbl-edit" onclick="openEditMatch(<?php echo e($match->id); ?>)" title="Edit">
      ...
      <div class="score-row">
        <div class="match-team-name"><?php echo e($match->team_a); ?></div>
        <?php if($match->status !== 'scheduled'): ?>
          <div class="score-center">
            <span class="score-num"><?php echo e($match->score_a); ?></span>
            <span class="score-vs">vs</span>
            <span class="score-num"><?php echo e($match->score_b); ?></span>
          </div>
        <?php else: ?>
          <div class="score-vs" style="font-size:14px;font-weight:600;color:var(--gray-400)">VS</div>
        <?php endif; ?>
        <div class="match-team-name" style="text-align:right"><?php echo e($match->team_b); ?></div>
      </div>
...
<div class="modal-backdrop hidden" id="matchModal">
  <div class="modal-box">
    ...
    <form method="POST" id="matchForm" action="<?php echo e(route('matches.store')); ?>">
      <?php echo csrf_field(); ?>
...
<?php echo $__env->make('layouts.app', \Illuminate\Support\Arr::except(get_defined_vars(), ['__data', '__path']))->render(); ?><?php /**PATH C:\Users\Default.DESKTOP-RDM1A2L\Downloads\IT9AL\laravel-tournament\resources\views\pages\matches.blade.php ENDPATH**/ ?>